...
public final class NamespacePersisted extends EventSourcedBehavior<NamespaceMessage, NamespaceEvent, NamespacePersisted.State> {
    ...
    private State onRepositoryCreated(State state, RepositoryCreated created) {
        ...
        Behavior<RepositoryMessage> repoBehavior = Repository.createBehavior(
            context, repositoryStorageAdapter, created.getNamespace(), created.getRepository(), created.getCreated());
        ...
        RepositoryTerminated terminated = RepositoryTerminated.apply(name, created.getRepository());
